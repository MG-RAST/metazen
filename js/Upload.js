...
function update_inbox (data, files, action) {
  if (data) {
	...
	if ((seq_dlist[dlist[i]] == 1) && inf['file type'] && (inf['file type'] == 'malformed')) {
	  ...
	  inbox_html += "<option style='display: none; padding-left: 35px; color: red;' value='"+dlist[i]+"/"+fn+"'>"+fn+"</option>";
	} else if ((seq_dlist[dlist[i]] == 1) && inf['bp count'] && inf['bp count'] < 10000) {
	  inbox_html += "<option style='display: none; padding-left: 35px; color: red;' value='"+dlist[i]+"/"+fn+"'>"+fn+"</option>";
	} else if (lock_msg != "") {
	...
	inbox_html += "<option style='color: red;'>"+flist[i]+"</option>";
      } else if (isSeq && inf['bp count'] && inf['bp count'] < 10000) {
	inbox_html += "<option style='color: red;'>"+flist[i]+"</option>";
      } else if (lock_msg != "") {
	...
	for (var h=0; h<DataStore['user_inbox'][user.login].fileinfo[i].length; h++) {
	  ...
	  if (fn.match(is_a_sequence_file_ending)) {
	    var inf = DataStore['user_inbox'][user.login].fileinfo[i+'/'+fn];
	      if (inf && inf['bp count'] && (inf['file type'] != 'malformed') && inf['unique id count'] && inf['sequence count'] && (inf['unique id count'] == inf['sequence count']) && (! inf['Error']) && inf['bp count'] > 10000) {
